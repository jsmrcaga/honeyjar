...
function factory(db){
	...
	Handler.prototype.__load = function(){
		console.log(`[HONEYJAR][HANDLER] Loading data for ${this.__type}`);
		return new Promise((resolve, reject) => {
			fs.readFile(this.__filename, {encoding: 'utf8'},  (err, raw) => {
				...
				let data = null;
				if(db.options.b64){
					return resolve(JSON.parse(Buffer.from(raw, 'base64').toString('utf8')));
				}

				return resolve(JSON.parse(Buffer.from(raw).toString('utf8')));
			});
		});
	};
